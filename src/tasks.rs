...
const READ_CACHE_SCAN_BUDGET_PER_SIDE: usize = 300;
const READ_CACHE_MIN_TICK_MS: u64 = 10;
const READ_CACHE_MAX_TICK_MS: u64 = 100;
const READ_CACHE_TARGET_P95_MS: u64 = 450;
const READ_CACHE_TARGET_MAX_MS: u64 = 700;
const READ_CACHE_FORCE_REFRESH_MS: u64 = 2000;
const READ_CACHE_MARKETS_PER_TICK_DEFAULT: usize = 3;
const READ_CACHE_CTRL_STEP_DOWN_MS: u64 = 5;
const READ_CACHE_CTRL_STEP_UP_MS: u64 = 10;
const ORDER_INTENT_BATCH_MAX: usize = 32;
...
const HISTORY_CLEAN_INTERVAL_SECS_DEFAULT: u64 = 60;
const HISTORY_INACTIVE_MARKET_TRADE_TTL_MS_DEFAULT: i64 = 30 * 60 * 1000;
const HISTORY_CLOSED_ORDER_MATCH_TTL_MS_DEFAULT: i64 = 24 * 60 * 60 * 1000;
...
fn read_cache_force_refresh_ms() -> u64 {
    std::env::var("READ_CACHE_FORCE_REFRESH_MS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(READ_CACHE_FORCE_REFRESH_MS)
        .clamp(250, 30_000)
}

fn read_cache_skip_if_order_intent_q_above() -> i64 {
    std::env::var("READ_CACHE_SKIP_IF_ORDER_INTENT_Q_ABOVE")
        .ok()
...
fn read_cache_markets_per_tick() -> usize {
    std::env::var("READ_CACHE_MARKETS_PER_TICK")
        .ok()
        .and_then(|v| v.parse::<usize>().ok())
        .unwrap_or(READ_CACHE_MARKETS_PER_TICK_DEFAULT)
        .clamp(1, 8)
}

fn history_clean_interval_secs() -> u64 {
    std::env::var("HISTORY_CLEAN_INTERVAL_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(HISTORY_CLEAN_INTERVAL_SECS_DEFAULT)
        .clamp(10, 3600)
}

fn history_inactive_market_trade_ttl_ms() -> i64 {
    std::env::var("HISTORY_INACTIVE_MARKET_TRADE_TTL_MS")
        .ok()
        .and_then(|v| v.parse::<i64>().ok())
        .unwrap_or(HISTORY_INACTIVE_MARKET_TRADE_TTL_MS_DEFAULT)
        .clamp(10_000, 7 * 24 * 60 * 60 * 1000)
}

fn history_closed_order_match_ttl_ms() -> i64 {
    std::env::var("HISTORY_CLOSED_ORDER_MATCH_TTL_MS")
        .ok()
        .and_then(|v| v.parse::<i64>().ok())
        .unwrap_or(HISTORY_CLOSED_ORDER_MATCH_TTL_MS_DEFAULT)
...
pub(crate) async fn prime_read_caches(state: &AppState) {
    ...
    for market_id in market_ids {
        state.market_read_cache.insert(
            market_id,
            MarketReadCache {
                market_id,
                best_bid_cents: 0,
...
async fn run_market_match_slice(state: &AppState, market_id: i16) {
    ...
    let (mut crossed, mut best_bid, mut best_ask) = if let Some(eng) = try_lock_read(&state.engine, "tasks.run_market_match_slice.engine_try_read") {
        (
    ...
    if crossed {
        if matched_total == 0 {
            if let Some(mut eng) = try_lock_write(&state.engine, "tasks.run_market_match_slice.engine_try_write") {
                ...
                best_bid = eng.best_live_bid(market_id);
                best_ask = eng.best_live_ask(market_id);
                crossed = matches!((best_bid, best_ask), (Some(b), Some(a)) if b >= a);
                if scrubbed > 0 {
                    state.mark_market_activity(market_id);
                    eprintln!(
...
pub(crate) fn start_background_tasks(
    ...
    let force_refresh_ms = read_cache_force_refresh_ms();
    ...
    let history_interval_secs = history_clean_interval_secs();
    let trade_ttl_ms = history_inactive_market_trade_ttl_ms();
    let order_match_ttl_ms = history_closed_order_match_ttl_ms();
    tokio::spawn(async move {
        loop {
            tokio::time::sleep(Duration::from_secs(history_interval_secs)).await;
            ...
            if let Some(mut eng) = try_lock_write(&s_clean.engine, "tasks.history_prune.engine_try_write") {
                let now_ms = crate::now_epoch_ms();
                let (markets_removed, order_matches_removed) =
                    eng.prune_history(now_ms, trade_ttl_ms, order_match_ttl_ms);
                if markets_removed > 0 || order_matches_removed > 0 {
                    eprintln!(
                        "[history_prune] markets_removed={} order_matches_removed={} trade_ttl_ms={} order_match_ttl_ms={}",
                        markets_removed, order_matches_removed, trade_ttl_ms, order_match_ttl_ms
                    );
                }
            }
        }
    ...
    tokio::spawn(async move {
        ...
        let skip_q_above = read_cache_skip_if_order_intent_q_above();
        let markets_per_tick = read_cache_markets_per_tick();
        eprintln!(
            "[read_cache] worker=0 tick_ms={} min_tick_ms={} max_tick_ms={} target_p95_ms={} target_max_ms={} force_refresh_ms={} markets_per_tick={} skip_if_q_above={}",
            initial_tick_ms, min_tick_ms, max_tick_ms, target_p95_ms, target_max_ms, force_refresh_ms, markets_per_tick, skip_q_above
        );
        loop {
            ...
            let order_intent_q = s_read.perf.order_intent_queue_len.load(Ordering::Relaxed);
            if skip_q_above > 0 && order_intent_q > skip_q_above {
                s_read
                    .perf
                    .read_cache_skipped_pressure
            ...
            // Refresh only markets that changed since their last cache build, or
            // have exceeded a low-frequency force-refresh interval.
            let start_idx = rr_idx % market_ids.len();
            let mut candidates: Vec<(i16, i64)> = Vec::with_capacity(market_ids.len());
            for step in 0..market_ids.len() {
                let idx = (start_idx + step) % market_ids.len();
                let mid = market_ids[idx];
                let (cache_updated_ms, age_ms) = if let Some(c) = s_read.market_read_cache.get(&mid) {
                    (c.updated_at_ms, now_ms.saturating_sub(c.updated_at_ms))
                } else {
                    (0, i64::MAX / 4)
                };
                let activity_ms = s_read
                    .market_activity_ms
                    .get(&mid)
                    .map(|v| *v.value())
                    .unwrap_or(0);
                let dirty = activity_ms > cache_updated_ms;
                let overdue = age_ms >= force_refresh_ms as i64;
                if !dirty && !overdue {
                    continue;
                }
                let score = if dirty {
                    age_ms.saturating_add(force_refresh_ms as i64)
                } else {
                    age_ms
                };
                candidates.push((mid, score));
            }
            ...
            let selected_ids: Vec<i16> = candidates
                .into_iter()
                .take(markets_per_tick)
                .map(|(mid, _)| mid)
                .collect();

            let mut cache_updates = None;
            for attempt in 0..2usize {
                if let Some(eng) = try_lock_read(&s_read.engine, "tasks.read_cache.engine_try_read") {
                    ...
                    for market_id in &selected_ids {
                        ...
                        updates.push((*market_id, cache));
                    }
                    cache_updates = Some(updates);
                    break;
                }
                if attempt == 0 {
            ...
            if cache_updates.is_none() {
                s_read.perf.read_cache_engine_busy.fetch_add(1, Ordering::Relaxed);
            }
            if let Some(updates) = cache_updates {
                for (market_id, cache) in updates {
    ...
    tokio::spawn(async move {
        loop {
            ...
            if expired > 0 {
                if let Some(markets) = try_lock_read(&s_exp.markets, "tasks.expiry.markets_try_read") {
                    for mid in markets.keys().copied() {
                        s_exp.mark_market_activity(mid);
                    }
                }
                            ...
                            for intent in &batch {
                                ...
                                match eng.reserve_for_order(j.user_id, j.market_id, j.buy, j.qty_units, j.price_cents) {
                                    Ok(()) => {
                                        ...
                                        s_mid.mark_market_activity(j.market_id);
                                    }
                                    Err(e) => {
            ...
            s_cancel
                .with_engine_write_profile("tasks.cancel.apply", |eng| {
                    ...
                    if let Some((uid, mid, buy, rem, px)) = rel {
                        ...
                        s_cancel.mark_market_activity(mid);
                    }
                })
                .await;
