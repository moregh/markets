...
pub(crate) fn checked_notional_cents(qty_units: i64, price_cents: i64) -> Option<i64> {
    qty_units.checked_mul(price_cents)
}

pub(crate) fn checked_fee_cents(qty_units: i64, price_cents: i64, fee_rate_ppm: i64) -> Option<i64> {
    let gross = checked_notional_cents(qty_units, price_cents)?;
    let scaled = gross.checked_mul(fee_rate_ppm)?;
    Some((scaled / 1_000_000).max(0))
}
...
impl EngineState {
    ...
    pub(crate) fn reserve_for_order(&mut self, user_id: i64, market_id: i16, buy: bool, qty: i64, price_cents: i64) -> Result<(), ApiError> {
        ...
        if buy {
            let required = checked_notional_cents(qty, price_cents)
                .ok_or_else(|| ApiError::new(StatusCode::BAD_REQUEST, "Order value overflow"))?;
    ...
    pub(crate) fn release_reservation(&mut self, user_id: i64, market_id: i16, buy: bool, qty: i64, price_cents: i64) {
        if let Some(mkt) = self.markets.get(&market_id).cloned() {
            let w = self.wallet_mut(user_id);
            if buy {
                let Some(required) = checked_notional_cents(qty, price_cents) else {
                    eprintln!(
                        "[engine] reservation_release_overflow user_id={} market_id={} qty={} price_cents={}",
                        user_id, market_id, qty, price_cents
                    );
                    return;
                };
    ...
    pub(crate) fn restore_order_to_book_front(&mut self, order_id: i64) {
        ...
        let restoring_key = match self.orders.get(&order_id) {
            Some(o) => (o.created_at, o.order_id),
            None => return,
        };
        let existing_ids: Vec<i64> = self
            ...
            .and_then(|book| {
                if buy {
                    book.buys.get(&price_cents)
                } else {
                    book.sells.get(&price_cents)
                }
            })
            .map(|level| level.q.iter().copied().collect())
            .unwrap_or_default();
        let insert_pos = existing_ids
            .iter()
            .position(|existing_oid| {
                let Some(existing) = self.orders.get(existing_oid) else { return false; };
                (existing.created_at, existing.order_id) > restoring_key
            })
        ...
        let pos = insert_pos.min(level.q.len());
        level.q.insert(pos, order_id);
        if buy {
            book.buy_count = book.buy_count.saturating_add(1);
        } else {
            book.sell_count = book.sell_count.saturating_add(1);
    ...
    pub(crate) fn apply_trade(&mut self, t: &Trade) {
        ...
        let Some(gross) = checked_notional_cents(t.qty_units, t.price_cents) else {
            eprintln!(
                "[matcher] arithmetic_overflow qty_units={} price_cents={} buy_order_id={} sell_order_id={}",
                t.qty_units, t.price_cents, t.buy_order_id, t.sell_order_id
            );
            return;
        };
...

        {
            ...
            let Some(release) = checked_notional_cents(t.qty_units, buy_limit_price) else {
                eprintln!(
                    "[matcher] arithmetic_overflow qty_units={} buy_limit_price={} buy_order_id={}",
                    t.qty_units, buy_limit_price, t.buy_order_id
                );
                return;
            };
            *rc = (*rc - release).max(0);
    ...
    pub(crate) fn recompute_reservations_from_open_orders(&mut self) {
        ...
        for o in open_orders {
            ...
            if o.buy {
                let Some(required) = checked_notional_cents(o.remaining, o.price_cents) else {
                    eprintln!(
                        "[engine] reservation_overflow order_id={} remaining={} price_cents={}",
                        o.order_id, o.remaining, o.price_cents
                    );
                    continue;
                };
                *w.res_cents.entry(mkt.currency_id).or_insert(0) += required;
    ...
    pub(crate) fn prune_history(
        &mut self,
        now_ms: i64,
        inactive_market_trade_ttl_ms: i64,
        closed_order_match_ttl_ms: i64,
    ) -> (usize, usize) {
        let mut remove_markets = Vec::new();
        for (mid, list) in &self.trades {
            let active_orders = self
                .books
                .get(mid)
                .map(|b| b.buy_count > 0 || b.sell_count > 0)
                .unwrap_or(false);
            if active_orders {
                continue;
            }
            let freshest_ts = list.front().map(|t| t.ts_ms).unwrap_or(0);
            if freshest_ts < (now_ms - inactive_market_trade_ttl_ms) {
                remove_markets.push(*mid);
            }
        }
        let removed_markets = remove_markets.len();
        for mid in remove_markets {
            self.trades.remove(&mid);
        }

        let mut remove_order_matches = Vec::new();
        for oid in self.order_matches.keys() {
            let keep = if let Some(o) = self.orders.get(oid) {
                if o.status == "OPEN" || o.status == "PARTIAL" {
                    true
                } else {
                    o.created_at.timestamp_millis() >= (now_ms - closed_order_match_ttl_ms)
                }
            } else {
                false
            };
            if !keep {
                remove_order_matches.push(*oid);
            }
        }
        let removed_order_matches = remove_order_matches.len();
        for oid in remove_order_matches {
            self.order_matches.remove(&oid);
        }

        (removed_markets, removed_order_matches)
    }
