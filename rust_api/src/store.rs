    ...
    state
        ...
        crate::EVT_ORDER_ACCEPTED => {
            ...
            if order_id > 0 && uid > 0 && qty_units > 0 && price_cents > 0 {
                // Rebuild reservation state from the ledger so wallet availability remains
                // consistent after restart/replay.
                if let Some(mkt) = eng.markets.get(&market_id).cloned() {
                    let w = eng.wallet_mut(uid);
                    if buy {
                        let required = qty_units.saturating_mul(price_cents);
                        *w.res_cents.entry(mkt.currency_id).or_insert(0) += required;
                    } else {
                        *w.res_units.entry(mkt.product_id).or_insert(0) += qty_units;
                    }
                }
