...
const WALLET_LOCK_SHARDS: usize = 4096;
...
async fn get_my_wallet(State(state): State<AppState>, headers: HeaderMap) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    let (w, cn, pn) = match (
        try_lock_read(&state.engine, "main.get_my_wallet.engine_try_read"),
...
async fn get_my_wallet_details(State(state): State<AppState>, headers: HeaderMap) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    let (w, user_order_ids, orders_by_id, cn, pn) = match (
        try_lock_read(&state.engine, "main.get_my_wallet_details.engine_try_read"),
...
async fn get_my_orders(State(state): State<AppState>, headers: HeaderMap) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    let rows: Vec<OrderResponse> = if let Some(eng) = try_lock_read(&state.engine, "main.get_my_orders.engine_try_read") {
...
async fn main() -> Result<()> {
    ...
    let state = AppState {
        ...
        wallet_mutexes: Arc::new((0..WALLET_LOCK_SHARDS).map(|_| Arc::new(Mutex::new(()))).collect()),
